...
In this second example, available as supplementary material file
\textbf{Multi120_Pilatus100k}, a single Pilatus module (100k) is mounted on the
arm which is moved from 5 to 65 degrees per half degree step.
...
calibrated using the pyFAI-calib tool presented in the previous section.
This manual calibration is rather unstable due to the small size of the
detector. 
The rotation of the detector ($rot_1=0$) has been fixed to have consistency in
the four images.

An initial set of parameters with $rot_2$ equal to the goniometer angle is
first converged, with some constrains and bounds (see supplementary materials).
Then all other images have been added to the model, and control points were 
automatically extracted from all images according to the inital model of
geometry.
After all constrains and bounds have been removed, the model was refined again
and used to generate a \textit{MultiGeometry} object. 
This integrator was used to generate the pattern shown in Figure \ref{bm20}, in
blue.
The peak of the curve all appear at the right position but the first ones looks
too small and too broad. 
...
close-up view on the first peak with a simple model (blue) or with an model
where two parameters are function of the goniometer position.}
\end{center}
\end{figure}


This is confirmed by displaying the first ring, in the image taken with the
goniometer at 10 degrees, in Figure\ref{bm20_10}. 
The expected position of the ring, in dashed red line, does not correspond
with the ring in green. 
The control points extracted are plotted in blue. 
...
Another model was suggested, allowing also the $rot_1$ to depend on the
goniometer position. 
After refinement, the residual error dropped by a factor two and the
low angle peaks were sharpen (Figure \ref{bm20} curve in orange). 
...
of magnitude faster than previously, making the beamline scientist pretty happy. 
