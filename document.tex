...
v0.14 which is to be released mid-2017 and still under development.
...
refers to polarization correction and $A$ refers to detector apparant efficiency
due to incidence photon angle (for integrating detectors, high energy photons
with larger incident angle see longer sensor thickness).
...
For non planar detectors (also supported by pyFAI) those ``rings'' can
...
Due to the azimuthal symmetry of the Debye-Scherrer cones, the $rot_3$ parameter
is never refined and stays at zero unless specified differently.  
This can be used to simulate the rotation of the
...
when dealing when very small area detectors are used (i.e. the ImXPAD S10 is
as small as 80x120 pixels and it is still usable).
...
(must be named for more than one moving motor). 
...
The initial \textit{geometry transformation} has been defined as rot2
\begin{verbatim}
from pyFAI.goniometer import GeometryTransformation
gt = GeometryTransformation(param_names = ["dist_offset", "dist_scale", 
                                        "poni1", "poni2", "rot1", "rot2"],
