...
detectors in tranmission mode, they are simply ellipses.
...
The strength of this parametrization is that it describes any detector position in
space. 
...
\caption{The peak-picking tool selects automatically groups of points close to the clicked peaks which needs then to be assigned the proper ring number.}
\end{center}
\end{figure}
\begin{figure}
\label{calib_4}
...
project \cite{silx_v0.5.0}, which stream-lines the development effort around data
analysis. 
...
This \textit{tranformation function} takes hence motor names, and model-parameter names 
...
This makes actually six $dof$ to be refined.
...
from 5 to 65 degrees by half degree steps on the ROBL beam-line
...
Finally we acknowledge Carsten Detlefs and the late Claudio Ferrero for their help and careful
