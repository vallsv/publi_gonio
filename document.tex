...
the calibration (Figure \ref{gui}). 
...
\label{gui}
\begin{center}
\includegraphics[width=9cm]{images/calibration-3-peak-picking.pdf}
\caption{Graphical user interface developed in PyQt5 for calibrating
static detector positions in SAXS and WAXS geometry.}
\end{center}
\end{figure}
...
small as 80x120 pixels and it is still useable).
...
The Jupyter notebook \cite{ipython} in supplementary materials 1 presents the
usage of this translation function together with the translation table
refinement. This example shows the \textsc{poni} is moving on the detector
plane by one milimeter per meter horizontally and four vertically.
This ``large'' verical deviation has been confirmed by the beamline staff and
is related to last focussing miror.
...
They gracefully provided beamtime and test-data to allow debuging this 
goniometer optimization tool.
We would also like to thank the French CNRS for financing the IR-DRX project
in 2015 and 2016 which acted as a catalyser on the goniometer refinement,
