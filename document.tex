...
\shorttitle{Calibration tools in pyFAI w/wo goniometer}
 ...
 \aff[b]{University Grenoble Alpes, 38000 \city{Grenoble}, \country{France}}
 \aff[c]{CNRS, Institut NEEL, 38000 \city{Grenoble}, \country{France}}
 \aff[c]{ESRF, The European Synchrotron, CS40220, 38043 \city{Grenoble}
 Cedex 9, \country{France}}
 \aff[d]{Helmholtz-Zentrum Dresden-Rossendorf, Institute of Resource Ecology, 
Bautzner Landstrasse 400, 01328 \city{Dresden}, \contry{Germany}}
...
\label{calib_1}
\begin{center}
\includegraphics[width=15cm]{images/1_experiment.png}
...
\label{calib_2}
\begin{center}
\includegraphics[width=15cm]{images/2_mask.png}
\caption{The mask drawing tool is used to exclude pixel from a threshold, or using a rectangular, polygonal or pencil selection.}
\end{center}
\end{figure}
\begin{figure}
\label{calib_3}
\begin{center}
\includegraphics[width=15cm]{images/3_picking.png}
\caption{The peak-picking tool selects automatically groups of points which needs to be assigned the proper ring number.}
\end{center}
\end{figure}
\begin{figure}
\label{calib_4}
\begin{center}
\includegraphics[width=15cm]{images/4_3d_view.png}
\caption{The geometry fitting tab can constrains the fit of the geometry. 
...
\label{calib_5}
\begin{center}
\includegraphics[width=15cm]{images/5_cake.png}
...
and caters for automatic ring extraction, fitting of the geometry and exporting
...
project \cite{silx_v0.5.0}, which stream lines development efforts around data
analysis. 
Diffraction image formats are still managed by the FabIO
library \cite{fabio_v0.5.0} which support dozens of file-formats.
...
modified or adapted.
The notbook presents the usage of the \textit{transformation function} class
...
ESRF ID29 beam-lines (Protein crystallography beamline, energy: 12.75 keV) from
...
This example is also available as Jupyter notebbok available
\cite{rotation_pilatus} and the workflow used is depicted in Figure \ref{workflow}.
...
from 5 to 65 degrees by half degree step on the ROBL beam-line
\cite{Kvashnina:vv5126},ESRF BM20.
In this experiment, 121 diffraction images of LaB$_6$
reference compound material have been acquired.
...
By moving the detector relative to the sample, one can remove the
effects of gaps which exist in pixel detectors.
